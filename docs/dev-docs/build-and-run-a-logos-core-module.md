...
1. Review the project directory. The generated project structure looks like this:
   ...
   :::info
   The `metadata.json` file is the single source of truth for your module. Read [LGX package format and bundling reference](https://docs.logos.co/core/reference/lgx-package-format-and-bundling-reference) for more details.
   :::
...
Before you can run your module with `logoscore` or install it into `logos-basecamp`, you need to package the build output into an `.lgx` package and install it into a `modules/` directory. Check out the [LGX package format and bundling reference](https://docs.logos.co/core/reference/lgx-package-format-and-bundling-reference) for more details on the format and bundling options.
...
:::tip
Check out [LGX package format and bundling reference](https://docs.logos.co/core/reference/lgx-package-format-and-bundling-reference) for more details on the format and bundling options.
:::
...
There are two Logos runtimes, `logoscore` and `logos-basecamp`, that can load and run your module. However, to interact with your module directly through the `logos-basecamp` interface, you need to [provide a UI module](https://docs.logos.co/core/build-modules/build-a-logos-cpp-ui-module).
...
:::tip
Check out [Logos CLI Reference](https://docs.logos.co/core/reference/logos-cli-reference) for more details on available commands and options.
:::

### Run with `logos-basecamp`

Logos [Basecamp](https://docs.logos.co/get-started/glossary#basecamp) is a desktop shell that provides a graphical interface for managing and running modules. Core modules run as background services in `logos-basecamp`. Other UI modules can call them through `LogosAPI` or the `logos.callModule()` bridge once they are installed.

:::warning
The LGX variant type must match the basecamp build type. Dev builds of basecamp expect dev LGX variants (for example, `darwin-arm64-dev`), and portable builds expect portable variants (for example, `darwin-arm64`). Check out the [LGX package format and bundling reference](https://docs.logos.co/core/reference/lgx-package-format-and-bundling-reference) for more details.
:::
...
:::tip
Try running the [Blockchain module](https://docs.logos.co/blockchain/get-started/run-a-logos-blockchain-node-from-cli), [Storage module](https://docs.logos.co/storage/get-started/run-logos-storage-node) or [Chat module](https://docs.logos.co/messaging/get-started/send-1-1-messages-logos-chat) or browse the full list of [Logos modules](https://github.com/logos-co/logos-modules#modules).
:::
